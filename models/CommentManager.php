...
class CommentManager extends AbstractEntityManager
{
    ...
    {
        $sql = "SELECT COUNT(*) as commentCount FROM comment WHERE id_article = :id_article";
        $result = $this->db->query($sql, ['id_article' => $articleId]);
        $commentCount = $result->fetch();
        //SI LA CONDITION EST VRAI AFFICHER COUNT SINON 0 (expression ternaire)
        //SYNTAXE EXEMPLE : (condition) ? value_if_true : value_if_false;
        return $commentCount ? (int)$commentCount['commentCount'] : 0;
    }
